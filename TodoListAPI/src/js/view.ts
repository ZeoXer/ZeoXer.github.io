...
const regist_pwdRepeat = document.getElementById(
  "regist-pwdRepeat"
)! as HTMLInputElement;
...
const regist_nickname_warning = document.getElementById(
  "regist-nickname-warning"
)!;
const regist_pwdRepeat_warning = document.getElementById(
  "regist-pwdRepeat-warning"
)!;
...
const category_not_completed = document.getElementById(
...
function showTodo(nickname: string) {
  login.style.display = "none";
  todo.style.display = "";
  login_email.value = "";
  login_pwd.value = "";
...
// 輸入格式檢查
function formatCheck(mode: number) {
...
function cleanWord(target: HTMLElement) {
  setTimeout(() => {
    target.textContent = "";
  }, 1000);
}
...
function showTodoItems(data: object[], state: showState) {
  let completed_str = "";
...
// 切換分類
function switch_category(state: showState) {
